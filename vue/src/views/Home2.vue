...
import points_cloud from '@/assets/points_cloud_v2.json';
  ...
  created() {
    ymaps.ready(() => {
      ...
      for (let i = 0; i < polygons.length; i++) {
        sites.push({index: i, group: polygons[i].class, x: polygons[i].xy[0], y: polygons[i].xy[1], weight: 1, desc: polygons[i].desc}); //Math.random() % 4 + 4});
      }
      ...
      for (var i = 0; i < cells.length; i++) {
        polyArr.push(new ymaps.Polygon([cells[i]], {
            balloonContent: cells[i].site.originalObject ? cells[i].site.originalObject.group : i,
          }, {
            // Фоновое изображение.
